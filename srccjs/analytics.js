...
var {getLocalDeps} = require("./asyncLocalDeps");
 ...
 */
function recordCalls(name, methodNames = []) {
  ...
  const methods = methodNames.length > 0 ? methodNames : Object.keys(original).filter(key => typeof original[key] === 'function');
        ...
        try {
          ...
          if (result instanceof Promise) {
            return result.then(value => {
              ...
              const duration = performance.now() - start;
              analytics.recordCall(name, method, args, {
                error: error.message
              }, duration);
              throw error;
            });
  ...
  const updatedDeps = {
    [name]: new Proxy(wrapped, handler)
  };
  (0, _asyncLocalDeps.addLocalDeps)(updatedDeps);
  return {
