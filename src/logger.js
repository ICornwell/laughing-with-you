...
export function createLogger(options) {
  ...
  if (options) {
    ...
    function format(level, message, context = {}) {
      ...
      const flevelColor = _getLevelColor(level);
      const fcontextStr = Object.keys(context).length > 0 ?
        ` ${JSON.stringify(context)}` : '';

      const formattedLevel = colorEnabled ?
        `${flevelColor}[${flevelStr}]\x1b[0m` : `[${flevelStr}]`;
    ...
    function log(level, message, context = {}) {
      ...
      if (buffering) {
        buffer.push({ level, message, context, formatted });
      } else {
    ...
    const logger ={
      debug: (message, context) => {debug(message, context); return this;},
      info: (message, context) => {info(message, context); return this;},
      warn: (message, context) => {warn(message, context); return this;},
      error: (message, context) => {error(message, context); return this;},       
