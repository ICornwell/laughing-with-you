...
'desribe' Is a factory that generates test fixtures, not tests. All the 'describe' sections are executed before /any/ test is run, which is why adding console logging into them can be really confusing when you try to run a single test and get logs about lots of others. As the 'describe' functions are run (which will happen on module loading if they are top module level). Their code, their nested 'describe's and other module level code will execute before any 'beforeXXX', 'afterXXX', 'it' or 'test' fucntions - describe execution containing these hooks just register the tests, their execution funtions and their set-up and tear-down functions. This is the discovery phase. Once everything has been discovered, the tests can be sorted and filtered and executed, with the relevant setups/teardowns. This is why the order looks weird or wrong, a describe runs first, whereever it is.
...
Code in a 'beforeAll' is not part of the discovery phase, it can not generate test function instances, if it is coded to go and find lots of data, and store it in a module level variable (or any other shared resource), any 'describe' function block above or beneath won't/can't use this. The describe has already run. If it tries to use a list created by a 'BeforeAll' to loop around an 'it', your logging might show the list in full detail, jest/vitest will then tell you there are no tests in the fixture. Ask me how I know.

Once you get all this (and apologise for patronising those that knew all along, and are thinking 'doh! what did you think it meant? moron.'), using it for building fixtures with absolute certainly of execution order and dynamic test generation and parameterisation is a joy.
