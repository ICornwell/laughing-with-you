...
var asyncLocalDeps = require("./asyncLocalDeps");
function proxyDep(dep, name) {
  //  console.log(`als: ${als}`)
  const handler = {
    get(target, prop, receiver) {
      const localDeps = (0, asyncLocalDeps.getLocalDeps)();

      // Then access the specific dependency by name, and the property on that dependency
      if (localDeps && localDeps[name] && typeof localDeps[name][prop] !== 'undefined') {
