 ...
 */
export function createRobustALS() {
  if (global.__appAls) {
 ...
 */
export async function runWithRobustALS(deps, callback) {
  ...
  try {
    ...
    try {
      ...
      return await als.run({ testContext: {}, dependencies: deps }, callback);
    } catch (objError) {
 ...
 */
export function getDepsRobustly() {
 ...
 */
export function setDepsRobustly(deps) {
  const als = global.__appAls || createRobustALS();
  const store = als.getStore();
  ...
  if (!store) {
    try {
      ...
      try {
        // Try plain object
        als.enterWith({ testContext: {}, dependencies: deps });
        return;
      } catch (objError) {
        // Last resort
        als.enterWith(deps);
