...
function logALSState(location) {
  ...
  const deps = (0, _asyncLocalDeps.getLocalDeps)(true); // pass true to suppress errors
...
describe('AsyncLocalStorage Test Diagnostics - Outer Suite', () => {
  ...
  test('Test with explicit setUpLocalDeps call', () => {
    ...
    (0, _asyncLocalDeps.setUpLocalDeps)({
      testValue: 'initialized in test 2'
    });
  ...
  describe('Nested test suite', () => {
    ...
    test('Nested test - checking ALS inheritance', () => {
      ...
      (0, _asyncLocalDeps.setUpLocalDeps)({
        nestedValue: 'initialized in nested test'
      });
  ...
  test('Test with runWithLocalDeps', async () => {
    ...
    await (0, _asyncLocalDeps.runWithLocalDeps)({
      runValue: 'inside run context'
    }, () => {
  ...
  test('Test direct manipulation of global.__appAls', () => {
    ...
    global.__appAls.run(new Map([['testContext', {}], ['dependencies', {
      directValue: 'direct'
    }]]), () => {
