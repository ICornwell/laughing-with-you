// Test for logger utility with Jest
...
describe('Logger', () => {
  async function createLoggerAndSpy(includeConsoleSpy = true) {
   ...
   const loggerConsole =  {
    ...
    const consoleSpy = includeConsoleSpy ? {
      debug: jest.spyOn(loggerConsole, 'debug').mockImplementation(() => { }),
      info: jest.spyOn(loggerConsole, 'info').mockImplementation(() => { }),
      warn: jest.spyOn(loggerConsole, 'warn').mockImplementation(() => { }),
      error: jest.spyOn(loggerConsole, 'error').mockImplementation(() => { })
    } : null;
  ...
  test('should get existing logger from dependencies', async () => {
    await withSignalMutex('mutexLoggerTests', async () => {
      ...
      setUpLocalDeps({ logger: initialLogger });
  ...
  test('should create a new logger if none exists in dependencies', async () => {
    await withSignalMutex('mutexLoggerTests', async () => {
      ...
      expect(getLocalDeps().logger).toBe(logger);
      //clearConsole(consoleSpy)
    }, 1000, 20)
  });
  ...
  test('should use test context for prefixed logging', async () => {
    await withSignalMutex('mutexLoggerTests', async () => {
      const { consoleSpy } = await createLoggerAndSpy();
      // Set up test context
      setUpLocalDeps({
        testContext: {
